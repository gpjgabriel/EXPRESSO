...
export default function PeriodChart({ apiData, loading }) {
  if (loading || !apiData) return <LoadingChart />

  const grouped = apiData.transactions.reduce((acc, item) => {
    const date = new Date(item.date).toLocaleDateString('pt-BR')
    if (!acc[date]) acc[date] = { receita: 0, despesa: 0 }
    acc[date].receita += item.type === 'RECEITA' ? item.value : 0
    acc[date].despesa += item.type === 'DESPESA' ? item.value : 0
    return acc
  }, {})
